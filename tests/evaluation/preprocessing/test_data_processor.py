...
class TestDataProcessorInit:
    ...
    def test_init_with_invalid_sample_duration(self, mock_load_data):
        ...
        with pytest.raises(ValueError, match=re.escape("Sample duration cannot exceed the recording duration.")):
            DataProcessor(
                prediction_directory_path="",
                annotation_directory_path="",
                sample_duration=15,
    ...
    def test_init_with_invalid_min_overlap(self, mock_load_data):
        ...
        with pytest.raises(ValueError, match=re.escape("Min overlap must be greater than 0.")):
            DataProcessor(
                prediction_directory_path="",
                annotation_directory_path="",
                min_overlap=-5,
        ...
        with pytest.raises(ValueError, match=re.escape("Min overlap cannot exceed the sample duration.")):
            DataProcessor(
                prediction_directory_path="",
                annotation_directory_path="",
    ...
    def test_init_with_invalid_recording_duration(self, mock_load_data):
        ...
        with pytest.raises(ValueError, match=re.escape("Recording duration must be greater than 0.")):
            DataProcessor(
                prediction_directory_path="",
                annotation_directory_path="",
...
class TestCreateTensors:
    ...
    def test_nan_in_confidence(self):
        ...
        with pytest.raises(ValueError, match=re.escape("NaN values found in confidence columns.")):
            self.dp.create_tensors()
...
class TestGetColumnName:
    ...
    def test_field_name_is_none(self):
        ...
        with pytest.raises(TypeError, match=re.escape("field_name cannot be None.")):
            self.dp.get_column_name(None, prediction=True)

    def test_prediction_is_none(self):
        """Test when prediction is None."""
        with pytest.raises(TypeError, match=re.escape("prediction parameter cannot be None.")):
...
class TestGetFilteredTensors:
    ...
    def test_selected_classes_not_in_data(self):
        ...
        with pytest.raises(ValueError, match=re.escape("No valid classes selected.")):
    ...
    def test_samples_df_is_empty(self):
        """Test when samples_df is empty."""
        self.dp.samples_df = pd.DataFrame()
        with pytest.raises(ValueError, match=re.escape("samples_df is empty.")):
